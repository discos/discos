...
void MedicinaActiveSurfaceBossImpl::initialize() throw (ACSErr::ACSbaseExImpl)
{
    ...
    boss->initialize();
	try {
        m_workingThread=getContainerServices()->getThreadManager()->create<CMedicinaActiveSurfaceBossWorkingThread,CSecureArea<CMedicinaActiveSurfaceBossCore> *>
		    ("MedicinaACTIVESURFACEBOSSWORKER",m_core);
	}
	...
	m_parser->add("asNewLutOn",new function0<CMedicinaActiveSurfaceBossCore,non_constant,void_type >(boss,&CMedicinaActiveSurfaceBossCore::asNewLutOn),0);
	m_parser->add("asNewLutOff",new function0<CMedicinaActiveSurfaceBossCore,non_constant,void_type >(boss,&CMedicinaActiveSurfaceBossCore::asNewLutOff),0);
...
//char *MedicinaActiveSurfaceBossImpl::command(const char *cmd) throw (CORBA::SystemException, ManagementErrors::CommandLineErrorEx)
{
	...
	try {
		...
		/*_ADD_BACKTRACE(ManagementErrors::CommandLineErrorExImpl,impl,ex,"MedicinaActiveSurfaceBossImpl::command()");
		impl.setCommand(cmd);
