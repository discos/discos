...
void CMedicinaActiveSurfaceBossCore::initialize()
{
	...
	m_profile = ActiveSurface::AS_PARABOLIC_FIXED;
	AutoUpdate = false;
...
void CMedicinaActiveSurfaceBossCore::reset (int circle, int actuator, int radius) throw (ComponentErrors::UnexpectedExImpl, ComponentErrors::CouldntCallOperationExImpl, ComponentErrors::CORBAProblemExImpl, ComponentErrors::ComponentNotActiveExImpl)
{
    if (circle == 0 && actuator == 0 &&radius == 0) // ALL
	{
		    ...
		    for (l = 1; l <= actuatorsInCircle[i]; l++)
			{
                if (!CORBA::is_nil(usd[i][l]))
				{
			    	try {
					    ...
					    _ADD_BACKTRACE(ComponentErrors::CouldntCallOperationExImpl,impl,E,"CMedicinaActiveSurfaceBossCore::reset()");
                        impl.setComponentName((const char*)usd[i][l]->name());
			            impl.setOperationName("reset()");
			            impl.log();
					}
					catch(CORBA::SystemException &E) {
                        _EXCPT(ComponentErrors::CORBAProblemExImpl,impl,"CMedicinaActiveSurfaceBossCore::reset()");
                        impl.setName(E._name());
                        impl.setMinor(E.minor());
			            impl.log();
                    }
		            catch(...) {
                        _EXCPT(ComponentErrors::UnexpectedExImpl,impl,"CMedicinaActiveSurfaceBossCore::reset()");
                        impl.log();
                    }
				}
				else {
                    _EXCPT(ComponentErrors::ComponentNotActiveExImpl,impl,"CMedicinaActiveSurfaceBossCore::reset()");
                    impl.log();
                }
			}
        }
        ...
        if (!CORBA::is_nil(usd[circle][actuator])) { // SINGLE ACTUATOR
            try {
                ...
                _THROW_EXCPT(ComponentErrors::UnexpectedExImpl,"CMedicinaActiveSurfaceBossCore::reset()");
            }
        }
        else {
            _THROW_EXCPT(ComponentErrors::ComponentNotActiveExImpl,"CMedicinaActiveSurfaceBossCore::reset()");
        }
    }
}
		...
		for (i = 1; i <= CIRCLES; i++)
			...
			for (l = 1; l <= actuatorsInCircle[i]; l++)
			{
				if (!CORBA::is_nil(usd[i][l]))
				{
						...
						_ADD_BACKTRACE(ComponentErrors::CouldntCallOperationExImpl,impl,E,"CMedicinaActiveSurfaceBossCore::onewayAction()");
                       				impl.setComponentName((const char*)usd[i][l]->name());
			            		impl.setOperationName("onewayAction()");
			            		impl.log();
					}
					catch(CORBA::SystemException &E) {
						_EXCPT(ComponentErrors::CORBAProblemExImpl,impl,"CMedicinaActiveSurfaceBossCore::onewayAction()");
						impl.setName(E._name());
						impl.setMinor(E.minor());
						impl.log();
					}
					catch(...) {
						_EXCPT(ComponentErrors::UnexpectedExImpl,impl,"CMedicinaActiveSurfaceBossCore::onewayAction()");
						impl.log();
					}
				}
				else {
                    			_EXCPT(ComponentErrors::ComponentNotActiveExImpl,impl,"CMedicinaActiveSurfaceBossCore::onewayAction()");
                    			impl.log();
                		}
				counter++;
				if (counter == 1116)
	...
	printf ("onewayaction SINGLE\n");
        if (!CORBA::is_nil(usd[circle][actuator])) { // SINGLE ACTUATOR
            try {
                ...
                _THROW_EXCPT(ComponentErrors::UnexpectedExImpl,"CMedicinaActiveSurfaceBossCore::oneWayAction()");
            }
        }
        else {
            _THROW_EXCPT(ComponentErrors::ComponentNotActiveExImpl,"CMedicinaActiveSurfaceBossCore::oneWayAction()");
        }
    }
}
...
void CMedicinaActiveSurfaceBossCore::watchingActiveSurfaceStatus() throw (ComponentErrors::CORBAProblemExImpl, ComponentErrors::CouldntGetAttributeExImpl, ComponentErrors::ComponentNotActiveExImpl)
{
    ...
    if (totalactuators >= 25 && totalactuators <= 72)  // 2 circle
        i= 2;
    if (totalactuators >= 49 && totalactuators <= 120)  // 3 circle
        i= 3;
    if (totalactuators >= 97 && totalactuators <= 168) // 4 circle
        i= 4;
    if (totalactuators >= 145 && totalactuators <= 216) // 5 circle
        i= 5;
    if (totalactuators >= 193 && totalactuators <= 264) // 6 circle
        i= 6;
    if (totalactuators >= 241 && totalactuators <= 268) // 7 circle
        i= 7;
...
void CMedicinaActiveSurfaceBossCore::sectorActiveSurface(int sector) throw (ComponentErrors::CORBAProblemExImpl, ComponentErrors::ComponentErrorsEx)
{
	if (sector == -1) {
	...
	while (usdTable >> lanIndexes[sector] >> circleIndexes[sector] >> usdCircleIndexes[sector] >> serial_usd >> graf >> mecc)
	{
		...
		{
			_ADD_BACKTRACE(ComponentErrors::CouldntGetComponentExImpl,Impl,ex,"CMedicinaActiveSurfaceBossCore::sectorActiveSurface()");
			Impl.setComponentName(serial_usd);
			Impl.log(LM_DEBUG);
		}
	}
...
void CMedicinaActiveSurfaceBossCore::setProfile(const ActiveSurface::TASProfile& newProfile) throw (ComponentErrors::ComponentErrorsExImpl)
{
	...
	{
        if (m_newLUT == false) {
            ifstream usdCorrections (USDDEFAULTTABLECORRECTIONS);
            if (!usdCorrections) {
                ACS_SHORT_LOG ((LM_INFO, "File %s not found", USDDEFAULTTABLECORRECTIONS));
                //exit(-1);
            }
            actuatorsCorrections.length(NPOSITIONS);
            for (i = 1; i <= CIRCLES; i++) {
...
void CMedicinaActiveSurfaceBossCore::asNewLutOn() throw (ComponentErrors::ComponentErrorsEx)
{
	int s, i, l;

    asOff();

    ifstream usdCorrections (USDNEWTABLECORRECTIONS);
    if (!usdCorrections) {
        ACS_SHORT_LOG ((LM_INFO, "File %s not found", USDNEWTABLECORRECTIONS));
        //exit(-1);
    }
    actuatorsCorrections.length(NPOSITIONS);
    for (i = 1; i <= CIRCLES; i++) {
...
void CMedicinaActiveSurfaceBossCore::asNewLutOff() throw (ComponentErrors::ComponentErrorsEx)
{
	int s, i, l;
...
void CMedicinaActiveSurfaceBossCore::asOn() throw (ComponentErrors::ComponentErrorsEx)
{
...
void CMedicinaActiveSurfaceBossCore::setActuator(int circle, int actuator, long int& actPos, long int& cmdPos, long int& Fmin, long int& Fmax, long int& acc,long int& delay/*CORBA::Long_out actPos, CORBA::Long_out cmdPos, CORBA::Long_out Fmin, CORBA::Long_out Fmax, CORBA::Long_out acc, CORBA::Long_out delay*/) throw (ComponentErrors::PropertyErrorExImpl, ComponentErrors::ComponentNotActiveExImpl)
{
	...
	ACS::ROpattern_var status_var;
    	...
    	if (!CORBA::is_nil(usd[circle][actuator])) {
        	try {
            		...
            		_EXCPT(ComponentErrors::CORBAProblemExImpl,impl,"CMedicinaActiveSurfaceBossCore::usdStatus4GUIClient()");
            		impl.setName(ex._name());
			impl.setMinor(ex.minor());
            		throw impl;
		}
        ...
        if (!local.isErrorFree()) {
            _ADD_BACKTRACE(ComponentErrors::CouldntGetAttributeExImpl,impl,local,"CMedicinaActiveSurfaceBossCore::usdStatus4GUIClient()");
			impl.setComponentName((const char*)usd[circle][actuator]->name());
...
void CMedicinaActiveSurfaceBossCore::checkAScompletionerrors (char *str, int circle, int actuator, CompletionImpl comp)
{
  printf ("%s %d_%d ", str, circle, actuator);
