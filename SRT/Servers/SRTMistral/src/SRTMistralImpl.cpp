// $Id: SRTMistralImpl.cpp,v 1.1 2011-03-14 14:15:07 c.migoni Exp $

#include "SRTMistralImpl.h"
...
static char *rcsId="@(#) $Id: SRTMistralImpl.cpp,v 1.1 2011-03-14 14:15:07 c.migoni Exp $";
...
SRTMistralImpl::SRTMistralImpl(const ACE_CString &CompName,maci::ContainerServices *containerServices) :
...
{
	AUTO_TRACE("SRTMistralImpl::SRTMistralImpl");
	m_initialized=false;
	m_senderThread=NULL;
	m_controlThread=NULL;

    m_MistralDB.initializing = false;
    m_MistralDB.ready = false;
    m_MistralDB.acquiring = false;
    m_MistralDB.sweeping = false;
    m_MistralDB.nop = false;
}

SRTMistralImpl::~SRTMistralImpl() 
{
	AUTO_TRACE("SRTMistralImpl::~SRTMistralImpl()");
...
void SRTMistralImpl::initialize() throw (ACSErr::ACSbaseExImpl)
{
	CError Err;
	CSenderThread::TSenderParameter threadPar;
	CCommandLine* line;
	//IRA::CString hostName,hostAddress;
	AUTO_TRACE("SRTMistralImpl::initialize()");
	ACS_LOG(LM_FULL_INFO,"SRTMistralImpl::initialize()",(LM_INFO,"COMPSTATE_INITIALIZING"));
	ACS_LOG(LM_FULL_INFO,"SRTMistralImpl::initialize()",(LM_INFO,"READING_CONFIGURATION"));
	// could throw an ComponentErrors::CDBAccessExImpl exception
	m_configuration.init(getContainerServices());
	ACS_LOG(LM_FULL_INFO,"SRTMistralImpl::initialize()",(LM_INFO,"CONFIGURATION_OK"));
	ACS_LOG(LM_FULL_INFO,"SRTMistralImpl::initialize()",(LM_INFO,"INITIALIIZING_COMMUNICATION_LINES"));
	...
	try {
		...
		_EXCPT(ComponentErrors::MemoryAllocationExImpl,dummy,"SRTMistralImpl::initialize()");
		throw dummy;
	}
	try {	
	...
	m_parser->add("targetSweep",new function0<CCommandLine,non_constant,void_type >(line,&CCommandLine::targetSweep),0 );
	m_parser->add("vnaSweep",new function0<CCommandLine,non_constant,void_type >(line,&CCommandLine::vnaSweep),0 );
	...
	try {
		...
		_ADD_BACKTRACE(ComponentErrors::ThreadErrorExImpl,_dummy,ex,"SRTMistralImpl::initialize()");
		throw _dummy;
	}
	catch (...) {
		_THROW_EXCPT(ComponentErrors::UnexpectedExImpl,"SRTMistralImpl::initialize()");
	}
	m_initialized=true;
	ACS_LOG(LM_FULL_INFO,"SRTMistralImpl::initialize()",(LM_INFO,"COMPSTATE_INITIALIZED"));
}

void SRTMistralImpl::execute() throw (ACSErr::ACSbaseExImpl)
{
	AUTO_TRACE("SRTMistralImpl::execute()");
	ACS::Time time;
	IRA::CError error;
	ACS_LOG(LM_FULL_INFO,"SRTMistralImpl::execute()",(LM_INFO,"BACKEND_INITIAL_CONFIGURATION"));
	try {
        ...
        m_ManagementStatus = Management::MNG_WARNING;
		_ADD_BACKTRACE(ComponentErrors::InitializationProblemExImpl,impl,ex,"SRTMistralImpl::execute()");
		throw impl;
	}	
	CSecAreaResourceWrapper<CCommandLine> line=m_commandLine->Get();
	ACS_LOG(LM_FULL_INFO,"SRTMistralImpl::execute()",(LM_INFO,"SOCKET_CONNECTING"));
	try {
		ACS_LOG(LM_FULL_INFO,"SRTMistralImpl::execute()",(LM_INFO,"HARDWARE_INITIALIZATION"));
		line->Init(&m_configuration);  	// this could throw an ACS exception.....
		//line->setTime(); // this could throw ACS exceptions....
	}
	catch (ACSErr::ACSbaseExImpl& ex) {
        m_ManagementStatus = Management::MNG_FAILURE;
		deleteAll();
		throw ex;
	}
	//resume the threads 
	...
	try {
        ...
        m_ManagementStatus = Management::MNG_WARNING;
		_ADD_BACKTRACE(ComponentErrors::ThreadErrorExImpl,__dummy,E,"SRTMistralImpl::execute");
		deleteAll();
		throw __dummy;
	}
    ...
    m_ManagementStatus = Management::MNG_OK;

    m_MistralDB.nop = true;

	ACS_LOG(LM_FULL_INFO,"SRTMistralImpl::execute()",(LM_INFO,"COMPSTATE_OPERATIONAL"));
}

void SRTMistralImpl::deleteAll()
{
	IRA::CError err;
	if (m_initialized) {
		...
		// ACS_LOG(LM_FULL_INFO,"SRTMistralImpl::deleteAll()",(LM_INFO,"DATA_LINE_CLOSED"));
		_IRA_LOGFILTER_FLUSH;
		_IRA_LOGFILTER_DESTROY;
		ACS_LOG(LM_FULL_INFO,"SRTMistralImpl::deleteAll()",(LM_INFO,"LOGS_FLUSHED"));
		m_initialized=false;
	}
}

void SRTMistralImpl::cleanUp()
{
	AUTO_TRACE("SRTMistralImpl::cleanUp()");
	stopPropertiesMonitoring();
	deleteAll();
	CharacteristicComponentImpl::cleanUp();	
}

void SRTMistralImpl::aboutToAbort()
{
	AUTO_TRACE("SRTMistralImpl::aboutToAbort()");
	deleteAll();
}

void SRTMistralImpl::sendHeader() throw (CORBA::SystemException, BackendsErrors::BackendsErrorsEx, 
		ComponentErrors::ComponentErrorsEx)
{
	AUTO_TRACE("SRTMistralImpl::sendHeader()");
	...
	/*try {
		...
		_EXCPT(ComponentErrors::UnexpectedExImpl,dummy,"SRTMistralImpl::sendHeader()");
		dummy.log(LM_DEBUG);
    ...
    */
	...
	try {
		...
		_ADD_BACKTRACE(BackendsErrors::TXErrorExImpl,impl,ex,"SRTMistralImpl::sendHeader()");
		impl.setDetails("main header could not be sent");
		impl.log(LM_DEBUG);
		throw impl.getBackendsErrorsEx();
	}
	catch (...) {
		_EXCPT(ComponentErrors::UnexpectedExImpl,impl,"SRTMistralImpl::sendHeader()");
		impl.log(LM_DEBUG);
	...
	#endif
	ACS_LOG(LM_FULL_INFO,"SRTMistralImpl::sendHeader()",(LM_INFO,"HEADERS_SENT"));
    ...
    m_MistralDB.acquiring = true;
}

void SRTMistralImpl::terminate() throw (CORBA::SystemException, BackendsErrors::BackendsErrorsEx,
		ComponentErrors::ComponentErrorsEx)
{
	AUTO_TRACE("SRTMistralImpl::terminate()");
	CSecAreaResourceWrapper<CCommandLine> line=m_commandLine->Get();
	try {
		...
		_EXCPT(ComponentErrors::UnexpectedExImpl,dummy,"SRTMistralImpl::terminate()");
		dummy.log(LM_DEBUG);
		throw dummy.getComponentErrorsEx();
	}
    m_MistralDB.acquiring = false;
...
void SRTMistralImpl::sendData(ACS::Time startTime) throw (CORBA::SystemException, BackendsErrors::BackendsErrorsEx,
		ComponentErrors::ComponentErrorsEx)
{

	AUTO_TRACE("SRTMistralImpl::sendData()");
	...
	try {
		...
		_EXCPT(ComponentErrors::UnexpectedExImpl,dummy,"SRTMistralImpl::sendData()");
		dummy.log(LM_DEBUG);
		throw dummy.getComponentErrorsEx();
	}
...
void SRTMistralImpl::sendStop() throw (CORBA::SystemException, BackendsErrors::BackendsErrorsEx,
		ComponentErrors::ComponentErrorsEx)
{
	
	AUTO_TRACE("SRTMistralImpl::sendStop()");
	...
	try {
		...
		_EXCPT(ComponentErrors::UnexpectedExImpl,dummy,"SRTMistralImpl::sendStop()");
		dummy.log(LM_DEBUG);
		throw dummy.getComponentErrorsEx();
	}
	...
	try {
		...
		_ADD_BACKTRACE(BackendsErrors::TXErrorExImpl,impl,ex,"SRTMistralImpl::sendStop()");
		impl.setDetails("stop message could not be sent");
		throw impl.getBackendsErrorsEx();
	}
	catch (...) {
		_EXCPT(ComponentErrors::UnexpectedExImpl,impl,"SRTMistralImpl::sendStop()");
		throw impl.getComponentErrorsEx();
	}
...
void SRTMistralImpl::setSection(CORBA::Long input,CORBA::Double freq,CORBA::Double bw,CORBA::Long feed,CORBA::Long pol,CORBA::Double sr,CORBA::Long bins) throw (
				CORBA::SystemException,ComponentErrors::ComponentErrorsEx,BackendsErrors::BackendsErrorsEx)
{
	AUTO_TRACE("SRTMistralImpl::setSection()");
	/*CSecAreaResourceWrapper<CCommandLine> line=m_commandLine->Get();
	try {
		...
		_EXCPT(ComponentErrors::UnexpectedExImpl,dummy,"SRTMistralImpl::setSection()");
		dummy.log(LM_DEBUG);
		throw dummy.getComponentErrorsEx();
	}*/		
}

void SRTMistralImpl::setTargetFileName (const char * fileName) throw (CORBA::SystemException,ComponentErrors::ComponentErrorsEx,
		BackendsErrors::BackendsErrorsEx)
{
	AUTO_TRACE("SRTMistralImpl::setTargetFileName()");
	CSecAreaResourceWrapper<CCommandLine> line=m_commandLine->Get();

	line->setTargetFileName(fileName);
}

ACS::doubleSeq *SRTMistralImpl::getRms() throw (CORBA::SystemException,
		ComponentErrors::ComponentErrorsEx,BackendsErrors::BackendsErrorsEx)
{
	AUTO_TRACE("SRTMistralImpl::getRms()");
	...
	try {
		...
		_EXCPT(ComponentErrors::UnexpectedExImpl,dummy,"SRTMistralImpl::getRms()");
		dummy.log(LM_DEBUG);
		throw dummy.getComponentErrorsEx();
	}*/			
	return rms._retn();
...
ACS::doubleSeq *SRTMistralImpl::getTpi() throw (CORBA::SystemException,
		ComponentErrors::ComponentErrorsEx,BackendsErrors::BackendsErrorsEx)
{
	AUTO_TRACE("SRTMistralImpl::getTpi()");
	...
	try {
		...
		_EXCPT(ComponentErrors::UnexpectedExImpl,dummy,"SRTMistralImpl::getTpi()");
		dummy.log(LM_DEBUG);
		throw dummy.getComponentErrorsEx();
	}*/			
	return tpi._retn();
...
ACS::doubleSeq * SRTMistralImpl::getZero () throw (CORBA::SystemException,
		ComponentErrors::ComponentErrorsEx,BackendsErrors::BackendsErrorsEx)
{
	AUTO_TRACE("SRTMistralImpl::getZero()");
	ACS::doubleSeq_var tpi=new ACS::doubleSeq;
	/*CSecAreaResourceWrapper<CCommandLine> line=m_commandLine->Get();
	...
	try {
		...
		_EXCPT(ComponentErrors::UnexpectedExImpl,dummy,"SRTMistralImpl::getZero()");
		dummy.log(LM_DEBUG);
		throw dummy.getComponentErrorsEx();
	}*/		
	return tpi._retn();
...
void SRTMistralImpl::setKelvinCountsRatio(const ACS::doubleSeq& ratio, const ACS::doubleSeq& tsys) throw (CORBA::SystemException)
{
	AUTO_TRACE("SRTMistralImpl::setKelvinCountsRatio()");
	//CSecAreaResourceWrapper<CCommandLine> line=m_commandLine->Get();
	//line->saveTsys(tsys,ratio);
	//if (m_senderThread) {
		//m_senderThread->setKelvinCountsRatio(ratio);
	//ACS_LOG(LM_FULL_INFO,"SRTMistralImpl::setKelvinCountsRatio()",(LM_INFO,"KELVIN_COUNTS_CONVERSION_FACTOR_SET"));
	//}
}

void SRTMistralImpl::enableChannels(const ACS::longSeq& enable) throw (CORBA::SystemException,ComponentErrors::ComponentErrorsEx,
		BackendsErrors::BackendsErrorsEx)
{
	AUTO_TRACE("SRTMistralImpl::enableChannels()");
	/*CSecAreaResourceWrapper<CCommandLine> line=m_commandLine->Get();
	try {
		...
		_EXCPT(ComponentErrors::UnexpectedExImpl,dummy,"SRTMistralImpl::enableChannels()");
		dummy.log(LM_DEBUG);
		throw dummy.getComponentErrorsEx();
	}*/			
}

void SRTMistralImpl::setTime() throw (CORBA::SystemException,ComponentErrors::ComponentErrorsEx,
		BackendsErrors::BackendsErrorsEx)
{
	AUTO_TRACE("SRTMistralImpl::setTime()");
	CSecAreaResourceWrapper<CCommandLine> line=m_commandLine->Get();
	try {
		line->setTime();
	}
		...
		_EXCPT(ComponentErrors::UnexpectedExImpl,dummy,"SRTMistralImpl::setTime()");
		dummy.log(LM_DEBUG);
		throw dummy.getComponentErrorsEx();
	}		
}

void SRTMistralImpl::setAttenuation(CORBA::Long input,CORBA::Double att) throw (CORBA::SystemException,ComponentErrors::ComponentErrorsEx,BackendsErrors::BackendsErrorsEx)
{
	AUTO_TRACE("SRTMistralImpl::setAttenutation()");
	/*CSecAreaResourceWrapper<CCommandLine> line=m_commandLine->Get();
	try {
		...
		_EXCPT(ComponentErrors::UnexpectedExImpl,dummy,"SRTMistralImpl::setAttenutation()");
		dummy.log(LM_DEBUG);
		throw dummy.getComponentErrorsEx();
	}*/
}

CORBA::Long SRTMistralImpl::getInputs(ACS::doubleSeq_out freq,ACS::doubleSeq_out bandWidth,ACS::longSeq_out feed,ACS::longSeq_out ifNumber) throw (CORBA::SystemException,
		ComponentErrors::ComponentErrorsEx,BackendsErrors::BackendsErrorsEx)
{
	long inputs;
	ACS::longSeq pol;
	AUTO_TRACE("SRTMistralImpl::getInputs()");
	CSecAreaResourceWrapper<CCommandLine> line=m_commandLine->Get();
...
void SRTMistralImpl::activateNoiseCalibrationSwitching(CORBA::Long interleave) throw (CORBA::SystemException,
		ComponentErrors::ComponentErrorsEx,BackendsErrors::BackendsErrorsEx)
{
	AUTO_TRACE("SRTMistralImpl::activateNoiseCalibrationSwitching()");
	/*CSecAreaResourceWrapper<CCommandLine> line=m_commandLine->Get();
	try {
		...
		_EXCPT(ComponentErrors::UnexpectedExImpl,dummy,"SRTMistralImpl::activateNoiseCalibrationSwitching()");
		dummy.log(LM_DEBUG);
		throw dummy.getComponentErrorsEx();
	}*/
}

void SRTMistralImpl::initialize(const char * configuration) throw (CORBA::SystemException,
		ComponentErrors::ComponentErrorsEx,BackendsErrors::BackendsErrorsEx)
{
	AUTO_TRACE("SRTMistralImpl::initialize()");
	CSecAreaResourceWrapper<CCommandLine> line=m_commandLine->Get();
	try {
		...
		_EXCPT(ComponentErrors::UnexpectedExImpl,dummy,"SRTMistralImpl::initialize()");
		dummy.log(LM_DEBUG);
		throw dummy.getComponentErrorsEx();
	}				
}

void SRTMistralImpl::setup() throw (CORBA::SystemException, ComponentErrors::ComponentErrorsEx, BackendsErrors::BackendsErrorsEx)
{
   AUTO_TRACE("SRTMistralImpl::setup()");
	CSecAreaResourceWrapper<CCommandLine> line=m_commandLine->Get();
	try {
		line->setup("MISTRAL");
	}
		...
		_EXCPT(ComponentErrors::UnexpectedExImpl,dummy,"SRTMistralImpl::setup()");
		dummy.log(LM_DEBUG);
		throw dummy.getComponentErrorsEx();
	}
}

void SRTMistralImpl::vnaSweep() throw (CORBA::SystemException, ComponentErrors::ComponentErrorsEx, BackendsErrors::BackendsErrorsEx)
{
   AUTO_TRACE("SRTMistralImpl::vnaSweep()");
	CSecAreaResourceWrapper<CCommandLine> line=m_commandLine->Get();
	try {
		line->setup("vnaSweep");
	}
		...
		_EXCPT(ComponentErrors::UnexpectedExImpl,dummy,"SRTMistralImpl::vnaSweep()");
		dummy.log(LM_DEBUG);
		throw dummy.getComponentErrorsEx();
	}
}

void SRTMistralImpl::targetSweep() throw (CORBA::SystemException, ComponentErrors::ComponentErrorsEx, BackendsErrors::BackendsErrorsEx)
{
   AUTO_TRACE("SRTMistralImpl::targetSweep()");
	CSecAreaResourceWrapper<CCommandLine> line=m_commandLine->Get();
	try {
		line->setup("targetSweep");
	}
	catch (ComponentErrors::ComponentErrorsExImpl& ex) {
		ex.log(LM_DEBUG);
		throw ex.getComponentErrorsEx();
		...
		_EXCPT(ComponentErrors::UnexpectedExImpl,dummy,"SRTMistralImpl::targetSweep()");
		dummy.log(LM_DEBUG);
		throw dummy.getComponentErrorsEx();
	}
}

void SRTMistralImpl::setIntegration(CORBA::Long Integration) throw (CORBA::SystemException,ComponentErrors::ComponentErrorsEx,
		BackendsErrors::BackendsErrorsEx)
{
	AUTO_TRACE("SRTMistralImpl::setIntegration()");
	/*CSecAreaResourceWrapper<CCommandLine> line=m_commandLine->Get();
	try {
		...
		_EXCPT(ComponentErrors::UnexpectedExImpl,dummy,"SRTMistralImpl::setIntegration()");
		dummy.log(LM_DEBUG);
		throw dummy.getComponentErrorsEx();
	}*/		
}

void SRTMistralImpl::getConfiguration (CORBA::String_out configuration) throw (CORBA::SystemException)
{
	AUTO_TRACE("SRTMistralImpl::getIntegration()");
	CSecAreaResourceWrapper<CCommandLine> line=m_commandLine->Get();
	try {
		...
		_EXCPT(ComponentErrors::UnexpectedExImpl,dummy,"SRTMistralImpl::getConfiguration()");
		dummy.log(LM_DEBUG);
		throw dummy.getComponentErrorsEx();
	}	
}

void SRTMistralImpl::getCommProtVersion (CORBA::String_out version) throw (CORBA::SystemException)
{
	AUTO_TRACE("SRTMistralImpl::getCommProtVersion()");
	CSecAreaResourceWrapper<CCommandLine> line=m_commandLine->Get();
	try {
		...
		_EXCPT(ComponentErrors::UnexpectedExImpl,dummy,"SRTMistralImpl::getCommProtVersion()");
		dummy.log(LM_DEBUG);
		throw dummy.getComponentErrorsEx();
	}	
}

CORBA::Boolean SRTMistralImpl::command(const char *cmd,CORBA::String_out answer) throw (CORBA::SystemException)
{
	AUTO_TRACE("SRTMistralImpl::command()");
	IRA::CString out;
	bool res;
...
_PROPERTY_REFERENCE_CPP(SRTMistralImpl,ACS::ROuLongLong,m_ptime,time);
_PROPERTY_REFERENCE_CPP(SRTMistralImpl,ACS::ROstring,m_pbackendName,backendName);
_PROPERTY_REFERENCE_CPP(SRTMistralImpl,ACS::ROdoubleSeq,m_pbandWidth,bandWidth);
_PROPERTY_REFERENCE_CPP(SRTMistralImpl,ACS::ROdoubleSeq,m_pfrequency,frequency);
_PROPERTY_REFERENCE_CPP(SRTMistralImpl,ACS::ROdoubleSeq,m_psampleRate,sampleRate);
_PROPERTY_REFERENCE_CPP(SRTMistralImpl,ACS::ROdoubleSeq,m_pattenuation,attenuation);
_PROPERTY_REFERENCE_CPP(SRTMistralImpl,ACS::ROlongSeq,m_ppolarization,polarization);
_PROPERTY_REFERENCE_CPP(SRTMistralImpl,ACS::ROlongSeq,m_pbins,bins);
_PROPERTY_REFERENCE_CPP(SRTMistralImpl,ACS::ROlong,m_pinputsNumber,inputsNumber);
_PROPERTY_REFERENCE_CPP(SRTMistralImpl,ACS::ROlong,m_pintegration,integration);
_PROPERTY_REFERENCE_CPP(SRTMistralImpl,ACS::ROpattern,m_pstatus,status);
_PROPERTY_REFERENCE_CPP(SRTMistralImpl,Management::ROTBoolean,m_pbusy,busy);
_PROPERTY_REFERENCE_CPP(SRTMistralImpl,ACS::ROlongSeq,m_pfeed,feed);
_PROPERTY_REFERENCE_CPP(SRTMistralImpl,ACS::ROdoubleSeq,m_ptsys,systemTemperature);
_PROPERTY_REFERENCE_CPP(SRTMistralImpl,ACS::ROlong,m_psectionsNumber,sectionsNumber);
_PROPERTY_REFERENCE_CPP(SRTMistralImpl,ACS::ROlongSeq,m_pinputSection,inputSection);

/* --------------- [ MACI DLL support functions ] -----------------*/
#include <maciACSComponentDefines.h>
MACI_DLL_SUPPORT_FUNCTIONS(SRTMistralImpl)
