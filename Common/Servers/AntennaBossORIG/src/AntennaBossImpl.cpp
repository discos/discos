// $Id: AntennaBossImpl.cpp,v 1.24 2011-04-15 09:09:11 a.orlati Exp $
...
static char *rcsId="@(#) $Id: AntennaBossImpl.cpp,v 1.24 2011-04-15 09:09:11 a.orlati Exp $";
...
void AntennaBossImpl::initialize() throw (ACSErr::ACSbaseExImpl)
{
	...
	m_parser->add<5>("source",new function5<CBossCore,non_constant,void_type,I<string_type>,I<rightAscension_type<rad,true> >,
			I<declination_type<rad,true> >,I<enum_type<EquinoxString,Antenna::TSystemEquinox > >,I<enum_type<SectionString,Antenna::TSections> > >(boss,&CBossCore::source));	
	...
	m_parser->add<4>("elevationScan",new function4<CBossCore,non_constant,void_type,I<rightAscension_type<rad,true> >,I<declination_type<rad,true> >,I<angleOffset_type<rad> >,I<interval_type> >(boss,&CBossCore::elevationScan));
...
void AntennaBossImpl::startScan(const char *targetName,ACS::Time& startUT,const Antenna::TTrackingParameters& parameters) throw (
		CORBA::SystemException,AntennaErrors::AntennaErrorsEx,ComponentErrors::ComponentErrorsEx)
{
	...
	CSecAreaResourceWrapper<CBossCore> resource=m_core->Get();
	try {
		resource->startScan(targetName,startUT,parameters);
	}
	catch (ComponentErrors::ComponentErrorsExImpl& ex) {
		ex.log(LM_DEBUG);
		throw ex.getComponentErrorsEx();
...
void AntennaBossImpl::elevationScan(CORBA::Double cRA,CORBA::Double cDec,CORBA::Double span,ACS::TimeInterval duration) throw (ComponentErrors::ComponentErrorsEx,AntennaErrors::AntennaErrorsEx,
		CORBA::SystemException)
{
	AUTO_TRACE("AntennaBossImpl::elevationScan()");
	CSecAreaResourceWrapper<CBossCore> resource=m_core->Get();
	double ra,dec,scanSpan;
	ACS::TimeInterval dur;
	try {
		ra=cRA; dec=cDec; scanSpan=span;
		dur=duration;
		resource->elevationScan(ra,dec,scanSpan,dur);
	}
	catch (ComponentErrors::ComponentErrorsExImpl& ex) {
		ex.log(LM_DEBUG);
		throw ex.getComponentErrorsEx();
...
void AntennaBossImpl::azimuthScan(CORBA::Double cRA,CORBA::Double cDec,CORBA::Double span,ACS::TimeInterval duration) throw (ComponentErrors::ComponentErrorsEx,AntennaErrors::AntennaErrorsEx,
		CORBA::SystemException)
{
	AUTO_TRACE("AntennaBossImpl::azimuthScan()");
	CSecAreaResourceWrapper<CBossCore> resource=m_core->Get();
	try {
		resource->azimuthScan(cRA,cDec,span,duration);
	}
	catch (ComponentErrors::ComponentErrorsExImpl& ex) {
		ex.log(LM_DEBUG);
		throw ex.getComponentErrorsEx();
...
bool AntennaBossImpl::checkScan(const char* targetName,ACS::Time startUt,const Antenna::TTrackingParameters& parameters,ACS::TimeInterval_out slewingTime) throw (
		ComponentErrors::ComponentErrorsEx,AntennaErrors::AntennaErrorsEx,CORBA::SystemException)
{
	...
	CSecAreaResourceWrapper<CBossCore> resource=m_core->Get();
	try {
		return resource->checkScan(startUt,targetName,parameters,slewingTime); 
	}
	catch (ComponentErrors::ComponentErrorsExImpl& ex) {
		ex.log(LM_DEBUG);
		throw ex.getComponentErrorsEx();
...
char *AntennaBossImpl::getGeneratorCURL(Antenna::TGeneratorType_out type) throw (CORBA::SystemException)
{
	...
	if (!CORBA::is_nil(resource->m_generator))  
