...
/* $Id: Schedule.h,v 1.14 2010-12-06 12:47:05 a.orlati Exp $										                                                            */
/*                                                                                                      */
...
public:
	 ...
	 * @param scan pointer to the buffer that stores the scan parameters, the number and the type of parameters could vary from scna type
	 *                to scan type.
	 * @param target name of the target if it exists
	 * @return false if the scan could not be found. 
	 */
	virtual bool getScan(const DWORD& id,Antenna::TGeneratorType& type,void *&scan,IRA::CString& target);
	
	/**
	 * This is the wrapper function of the previuos one, used to get scan data usign a structure.
	 * @param id identifier of the scan we are asking for 
...
private:
	...
	/**
	 * translate a string into a generator type defined in the Antenna package
	 * @return false if the translation was not possible
	 */
	bool string2ScanType(const IRA::CString& val,Antenna::TGeneratorType& type);
	 ...
	 * @param id numeral identifier of the scan
	 * @param sourceName name of the target source
	 */
	bool parseSidereal(const IRA::CString& val,Antenna::TTrackingParameters *scan,DWORD& id,IRA::CString& sourceName);
