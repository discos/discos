...
bool CScanList::parseSidereal(const IRA::CString& val,Antenna::TTrackingParameters *scan,DWORD& id)
{
	int start=0;
	IRA::CString token;
	...
	scan->targetName=(const char *)token;
		...
		if ((frame==Antenna::ANT_EQUATORIAL) && (ok)) {
			...
			if (frameOpen) {
				if (counter==0) { //first two parameters are lon and lat
					...
					if (!IRA::CIRATools::strToEquinox(token,scan->equinox)) return false;
					counter++;
				}
...
bool CScanList::parseOTFC(const IRA::CString& val,Antenna::TTrackingParameters *scan,Antenna::TTrackingParameters *secScan,DWORD& id)
{
	TRecord rec;
	...
	if ((out!=9) && (out!=12)) {   //parameters are 9 for the OTF plus 3 (not mandatory) for the offsets...
		...
		//coordFrame
		if (!IRA::CIRATools::strToCoordinateFrame(coordFrame,frame)) return false;
		...
		//subScan Frame
		if (!IRA::CIRATools::strToCoordinateFrame(subScanFrame,frame)) return false;
	...
	if (!getScan(scanCenter,rec)) return false; // the secondary scan does not exist
	Antenna::TTrackingParameters *tmp=static_cast<Antenna::TTrackingParameters *>(rec.primaryParameters);
	secScan->targetName=tmp->targetName;
