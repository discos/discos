...
public:
	...
	*/
	USDDevIO(USDImpl& pusd, ActiveSurface::lan_proxy& plan,BYTE addr,BYTE cmd,BYTE wLen,BYTE rLen) : m_pusd(pusd),m_pLan(plan),m_addr(addr),m_cmd(cmd),m_wLen(wLen),m_rLen(rLen),m_value(0)
...
private:
	...
	ActiveSurface::lan_proxy& m_pLan;   // component proxy
