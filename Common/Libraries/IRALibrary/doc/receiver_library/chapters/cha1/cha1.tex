...
\emph{master-slave}\index{master-slave}, ed in questo caso vi saranno dei campi di controllo ridondanti, 
oppure complessa come quella \emph{multi-master}\index{multi-master}, dove dispositivi diversi possono condividere le medesime risorse.
...
elencati di seguito:
\begin{itemize}
\item \texttt{CMD\_INQUIRY}\index{comando!inquiry}: chiede allo slave di restituire l'identificativo e 
...
\item \texttt{CMD\_VERSION}\index{comando!version}: chiede allo slave di restituire il codice identificativo. Il codice restituito dipende dall'hardware
...
\item \texttt{CMD\_GET\_ADDR}\index{comando!get address}: chiede allo slave di restituire il suo indirizzo di protocollo.
...
\item \texttt{CMD\_GET\_TIME}\index{comando!get time}: chiede allo slave di restituire la data e l'orario del suo orologio interno nella seguente
...
sequenza: secolo, anno, mese, giorno, ora, minuti, secondi e centesimi di secondo.
\item \texttt{CMD\_GET\_FRAME}\index{comando!get frame}: chiede allo slave di restituire la dimensione massima del campo PARAMETER (per i comandi)
...
\item \texttt{CMD\_GET\_PORT}\index{comando!get port}: chiede allo slave di restituire l'impostazione della porta, ovvero i parametri di configurazione
...
necessari a definire il funzionamento; una porta parallela, ad esempio, necessita la configurazione della direzione 
(ingresso/uscita) dei vari bit che costituiscono la parola.
\item \texttt{CMD\_GET\_DATA}\index{comando!get data}: chiede allo slave di restituire i dati presenti sulla porta; per una porta parallela, ad esempio,
...
La \texttt{MicroControllerBoard} \`e una libreria \emph{thread-safe}\index{thread safe} composta da~3 file: 
...
\item il \emph{checksum}\index{checksum} (se richiesto) deve essere corretto.
\end{itemize}
