...
/* $Id: SP_parser.h,v 1.24 2011-06-05 14:43:49 a.orlati Exp $											*/
/*                                                                                                      */
 ...
 * This library provides support for three kind of commands:
 ...
 * There is no prevention of name clashes, in case a command name is duplicated the conflict is resolved by calling first one that has was added, indipendetly if it is a simple command or a procedure.
 ...
 * It could throw an execution error of a package error
 */
#define _SP_REMOTECALL(X) IRA::CString (OBJ::*X)(const IRA::CString& command,const IRA::CString& package,const long& param)  throw (ParserErrors::ExecutionErrorExImpl,ParserErrors::PackageErrorExImpl)
/**
 * Prototype of the callback function, invoked when a injected procedure has completed. If accepts the name of the procedure and te execution result
 */
 ...
 * }
 * </pre>
 * For example a parser like this can succesfully interpret and execute commands like :
 * <pre>
 * modulo=12,33
 * date
 * random
 ...
 * </pre>
 * This class is partially thread safe, synchronization is done for the execution list on which the internal thread and the caller could operate simultanously. 
 * The rules list is not serialized, but it can be considered safe provided that no calls to <i>add()</i> methods are done during normal parser execution. This should not be
...
public: 
	 ...
	 * @param delim parameter separtor character
	 * @param cmdDelimiter character that separates the command from the parameters list
	 * @param asyncSupport true if asynchronous cammands (timetagged or not) are to be supported
	 ...
	 */  
	CParser(OBJ *obj,const BYTE& max,bool asyncSupport=false,const char delim=',',const char cmdDelimiter='=',const char timeDelimiter='@',const char jolly='*',const char answerDelimiter='\\',const char errorDelimiter='?') {
		m_delimiter=delim;
	...
	*/
	 ...
	 *               template parameter.
	 */
	template <int N> void add(const IRA::CString& fName,CBaseFunction *function) {
	...
	void add(const IRA::CString& name,const IRA::CString& package,const ACS::stringSeq& procedure) {
		TRule *elem=new TRule(procedure,name,package);
		m_ruleSet.push_back(elem);
	}
	
	/**
	 * This function adds directly a sequence of commands into the execution list. The sequence of commands will be executed exaclty as a procedure even if it has never been added
	 * in the rules list. After the execution the parser will not keep track of it anymore. 
	 * @param name name of the procedure, in this case it is not used to search for the procedure in the rule list, but is is simply a placeholder
	 * @param procedure this is the significant paramters, it stores the sequence of one or more commands (also procedures) that copose the procedure
	 * @param callBack pointer to th ecall back function. This will be called after the last procedure instruction is executed. If the last command is a timetagged command the call back
	 * @param callBackParam pointer to the parameter that will be passed to the callBack function
	 * is called exactly after the event has been scheduled in the timer, the real command execution is not waited
	 * @throw ParserErrors::ProcedureErrorExImpl in case the procedure does not pass the pre parsing stage.
	 */
	void inject(const IRA::CString& name,const ACS::stringSeq& procedure,_SP_CALLBACK(callBack)=NULL,const void *callBackParam=NULL) throw (ParserErrors::ProcedureErrorExImpl);
	 ...
	 * @thorw ParserErrors::ParserErrorsExImpl 
	 * @param command string that stores the command to be executed.
	 * @param out  the result of the requested command
	 */
	void run(const IRA::CString& command,IRA::CString& out) throw (ParserErrors::ParserErrorsExImpl);
...
private:
	/**
	 * This enlists the various command that the arser is able to manage
	 */
	enum TCommandType {
		COMMAND,     				/*!< this is a pure command, it is executed locally  */ 
		REMOTECOMMAND,     /*!< this commnand requires a remote invokation */
		PROCEDURE                  /*!< this a complex command, usually composed by more than one command or procedure */
	} ;
	class TRule {
	public:		
		TRule(CBaseFunction * const func,const IRA::CString& name,const int& arity): m_func(func),m_name(name),m_arity(arity),m_remoteFunc(NULL),m_package(""),m_param(0),m_type(COMMAND) {
			m_procedure.length(0);
		}
		TRule(_SP_REMOTECALL(remotefunc),const IRA::CString& name,const IRA::CString package,const long& prm): m_func(NULL),m_name(name),m_arity(0),m_remoteFunc(remotefunc),m_package(package),m_param(prm),m_type(REMOTECOMMAND) {
			m_procedure.length(0);
		}
		TRule(const ACS::stringSeq& procedure,const IRA::CString& name,const IRA::CString& package): m_func(NULL),m_name(name),m_arity(0),m_remoteFunc(NULL),m_package(package),m_param(0),m_type(PROCEDURE) {
	 ...
	 * character.. In case of errors  an execption is thrown. Iif the command has return values, they are returned as a list of values separeted by the 
	 ...
	 * @throw ParserErrors::ExecutionErrorExImpl if an error was encountered during the execution of the command (execution time error). The
	 ...
	 *               arguments), the list is separetated by the <i>answerDelimiter</i>. In case of exception the value is not meaningful.
	 */
	 ...
	 * @param timeCommand if <i>timeMark</i> is true, it contains the command line epurated with the time mark.
	 ...
	 * @return the number of parameters, if negative the parsing does not suceed.
	 */
	 ...
	 * @param timeCommand command line depurated by the time mark
	 ...
	 */
	TRule *checkCommand(const IRA::CString& line,IRA::CString& instr, IRA::CString* inParams,WORD& parNum,bool& timeTagged,IRA::CString& timeCommand,ACS::Time& execTime,ACS::TimeInterval& execInterval) const throw (
	 ...
	 */ 
	TRule *locate(const IRA::CString& instr) const;
	
	/**
	 * Push a command in the command list, the command list  will be executed by the thread according a FIFO
	 * @param line command line to be executed, with al lthe required arguments and extra symbol, time makere for example.
	 * @param package name of the packate that the command belongs to
	 ...
	 * @param parameter parameter passed as argument to the callback function
	*/
	void pushProcedure(const IRA::CString& name,const ACS::stringSeq& procedure,_SP_CALLBACK(callBack)=NULL,const void* parameter=NULL) throw (ParserErrors::ParserErrorsExImpl);
	...
	/**
	 * Static function called by the timer when the event expires in order to clean hadler parameters
	 */
